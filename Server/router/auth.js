...
router.get("/awsdata", async (req, res) => {
  const params = {
    TableName: "ECG_data",
  };
  ...
  DynamoDB.scan(params, function (err, data) {
    if (err) {
      console.error("Unable to find ECG data", err);
    } else {
      console.log(`Found ${data.Count} ECG Readings`);
      // data.Items.forEach(o => console.log(o.data));
      res.send(data.Items);
    }
  });
});
...
// add patient
router.post("/api/Patient-Data", async (req, res) => {
  const { userid, name, age, email, phone, dob, disease, doctor, hospital, deviceid, details } = req.body;
  console.log(req.body);

  if (!userid || !name || !email || !age || !phone || !dob || !disease || !doctor || !hospital || !deviceid) {
    return res
      .status(422)
      .json({ error: "plz fill all the fields properly!! " });
  }
  ...
  try {
    ...
    const patient = new Patient({
      userid, name, age, email, phone, dob, disease, doctor, hospital, deviceid, details, date
    });
    await patient.save();
    res.status(201).json({ message: "Data Savd Successfully!!" });
    // } else {
...
// all patients data
router.get("/api/allpatients", (req, res) => {
  Patient.find({})
...
router.post("/patient/details", async (req, res) => {
  ...
  if (!patientid) {
    return res.status(422).json({ error: "Error in Getting Patient Report" });
  }
...
router.get("/api/count", (req, res) => {
  Patient.countDocuments().then((count) => {
...
router.post("/SmtpEmail", (req, res) => {
  ...
  var sender = nodemailer.createTransport({
    service: 'gmail',
    auth: {
      user: 'tabletennissgfi@gmail.com',
      pass: 'aqcemvqhohchtjbb'
    }
  });

  var mail = {
    from: "tabletennissgfi@gmail.com",
    to: `${email}`,
    subject: `${subject}`,
    text: `${message}`,
    html:
      "<h1>PFA Patient Report</h1> ",
    attachments: [
      {
        filename: `${file}`,
  ...
  sender.sendMail(mail, function (error, info) {
    if (error) {
      ...
      console.log("Email sent successfully: "
        + info.response);
      return res.status(200).json({ error: "Mail Sended Successfully!!!" });
    }
  });
});
...
//   return mailer
//     .sendMail("anjana.saini@netleon.com", [req.body.email], req.body)
